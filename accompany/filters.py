...
class AccompanyFilter(django_filters.FilterSet):
    city = django_filters.CharFilter(method="filtered_city", lookup_expr='icontains', label="searching_city", widget=forms.TextInput(attrs={'placeholder' : '장소 검색으로 동행 찾기 (ex. 프랑스 파리)'}))
    ...
    def filtered_city(self, queryset, name, value):
        return queryset.filter(
            city__icontains=value 
        ) | queryset.filter(
            markers__icontains=value  
        )
